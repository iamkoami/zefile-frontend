...
import { CreditCard, SmartphoneDevice, CheckCircle, XmarkCircle, Crown, Sparks, NavArrowDown, RefreshDouble, InfoCircle } from 'iconoir-react';
...
type CheckoutStep = 'country' | 'method' | 'phone' | 'processing' | 'success' | 'failed';
...
export function SubscriptionCheckoutPanel() {
  ...
  // Country selection state - use country from pricing page if available
  const [selectedCountry, setSelectedCountry] = useState<string>(
    checkoutData?.countryCode || 'DEFAULT'
  );
  ...
  const currentAmount = checkoutData
    ? getTierPriceMinorUnits(checkoutData.tier, checkoutData.billingPeriod, selectedCountry)
    : 0;
  ...
  const startPolling = (reference: string) => {
    pollingRef.current = setInterval(async () => {
      try {
        const response = await subscriptionApi.getSubscriptionPaymentStatus(reference);
        if (response.data) {
  ...
  const handleSubmitPhone = async () => {
    if (!isPhoneValid || !checkoutData || !selectedMethod || !user?.email) return;
  ...
  const getBillingText = () => {
    return checkoutData?.billingPeriod === 'annual' ? tSub('perYear') : tSub('perMonth');
  };
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                {checkoutData.billingPeriod === 'annual' ? tSub('billedAnnually') : tSub('billedMonthly')}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="relative">
            ...
            >
              <span className="flex items-center gap-2">
                <span className="text-lg">{COUNTRY_CONFIG[selectedCountry]?.flag || COUNTRY_CONFIG.DEFAULT.flag}</span>
                {COUNTRY_CONFIG[selectedCountry]?.name || COUNTRY_CONFIG.DEFAULT.name}
              </span>
              <NavArrowDown className={`w-5 h-5 text-gray-500 transition-transform ${isCountryDropdownOpen ? 'rotate-180' : ''}`} />
            </button>
            ...
            {isCountryDropdownOpen && (
              <>
                ...
                <div
                  ...
                  role="listbox"
                >
                  {[...SUPPORTED_COUNTRIES, 'DEFAULT' as const].map((code) => (
                    <button
                      key={code}
                      ...
                      className={`w-full px-4 py-3 text-left text-sm hover:bg-gray-50 first:rounded-t-lg last:rounded-b-lg flex items-center gap-2 ${
                        selectedCountry === code ? 'bg-[#5E53E0]/5 text-[#5E53E0] font-medium' : 'text-gray-700'
                      }`}
                      role="option"
          ...
          {selectedMethod && (
            <div className={`mt-4 p-4 rounded-lg flex items-start gap-3 ${
              ...
              <InfoCircle className={`w-5 h-5 flex-shrink-0 mt-0.5 ${
                selectedMethod.type === 'card' ? 'text-green-600' : 'text-amber-600'
              }`} />
