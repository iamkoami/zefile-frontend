    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="relative h-6 mt-1">
          ...
          <p
            className={`absolute inset-0 text-sm truncate transition-all duration-200 ${
              isSelected ? 'text-gray-700' : 'text-gray-500'
            } ${
              isActive && !selectionMode ? 'opacity-0 translate-y-1' : 'opacity-100 translate-y-0'
            }`}
          >
            {t('sentOn', { date: formatDate(createdDateStr) })} - {formatSize(totalSize)} ({fileCountText}) - {downloadStatus}
            {' - '}
