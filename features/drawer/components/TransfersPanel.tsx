...
const TransfersPanel: React.FC = () => {
  ...
  const { isOpen, view, payload, pushView } = useDrawerStore();
  const {
    isSelectionMode,
    toggleSelection,
    selectAll,
  ...
  // Fetch transfers when drawer opens or view changes to transfers
  useEffect(() => {
    if (isOpen && view === "transfers") {
      fetchTransfers();
    }
  }, [isOpen, view, fetchTransfers]);
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <LoadingPanel message={t("loading")} />
      </div>
    );
  }
      ...
      <div className="relative  mt-6 mb-10">
        <div className="flex items-center justify-between pb-3">
          ...
          <div className="flex items-center gap-1">
            ...
            >
              {sortAscending ? (
                <ArrowUpNarrowWide className="w-5 h-5" strokeWidth={1.5} />
              ) : (
                <ArrowDownNarrowWide className="w-5 h-5" strokeWidth={1.5} />
              )}
            </button>
